...
const getMerchants = async (req, res) => {
  const response = await pool.query('SELECT * FROM merchants ORDER BY id DESC')
  res.status(200).json(response.rows)
}
...
const getMerchantById = async (req, res) => {
  const id = req.params.id
  const response = await pool.query('SELECT * FROM merchants WHERE id = $1', [id])
  res.json(response.rows)
}

const createMerchant = async (req, res) => {
    const { name, email } = req.body
    const response = await pool.query('INSERT INTO merchants (name, email) VALUES ($1, $2)', [name, email])
    console.log(response)
    res.send('created')
}
...
const updateMerchant = async (req, res) => {
    const id = req.params.id
    const { name, email } = req.body
    const response = await pool.query('UPDATE merchants SET name = $1, email = $2 WHERE id = $3', [
        name,
        email,
        id
    ])
...
const deleteMerchant = async (req, res) => {
  const id = req.params.id
  const response = await pool.query('DELETE FROM merchants WHERE id = $1', [id])
  console.log(response)
